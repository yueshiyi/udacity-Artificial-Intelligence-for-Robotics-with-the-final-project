...
setp_cost = [1, 1, 1, 1, 1.5, 1.5, 1.5, 1.5]
...
def plan(warehouse, dropzone, todo):
    cost = 0
    warehouse = numpy.array(warehouse)
    if numpy.where(warehouse=='x'): warehouse[numpy.where(warehouse=='x')] = 0
    warehouse = warehouse.astype(int)
    grid = numpy.copy(warehouse)
    grid[numpy.where(grid!=0)] = 1
    init = dropzone

    for i in range(len(todo)):
        goal = numpy.where(warehouse==todo[i])
        grid[goal[0], goal[1]] = 0

        task_distance = search(grid,init,goal,setp_cost)
        cost += 2 * task_distance
