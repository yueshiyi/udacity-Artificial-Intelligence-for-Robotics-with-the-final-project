...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(len(data)):
        ...
        latter = i+1
        ######    x
        Omega.value[2*former][2*former] += 1.0/motion_noise
        Omega.value[2*former][2*latter] += -1.0/motion_noise
        Omega.value[2*latter][2*former] += -1.0/motion_noise
        Omega.value[2*latter][2*latter] += 1.0/motion_noise
        ######    y
        Omega.value[2*former+1][2*former+1] += 1.0/motion_noise
        Omega.value[2*former+1][2*latter+1] += -1.0/motion_noise
        Omega.value[2*latter+1][2*former+1] += -1.0/motion_noise
        Omega.value[2*latter+1][2*latter+1] += 1.0/motion_noise

        ######    x
        Xi.value[2*former][0] += -dx/motion_noise
        Xi.value[2*latter][0] += dx/motion_noise
        ######    y
        Xi.value[2*former+1][0] += -dy/motion_noise
        Xi.value[2*latter+1][0] += dy/motion_noise

        ######     z
        for j in range(len(Z)):
            landmark_idx = Z[j][0]
            latter = N + landmark_idx
            Omega.value[2 * former][2 * former] += 1.0/measurement_noise
            Omega.value[2 * former][2 * latter] += -1.0/measurement_noise
            Omega.value[2 * latter][2 * former] += -1.0/measurement_noise
            Omega.value[2 * latter][2 * latter] += 1.0/measurement_noise
            Omega.value[2 * former + 1][2 * former + 1] += 1.0/measurement_noise
            Omega.value[2 * former + 1][2 * latter + 1] += -1.0/measurement_noise
            Omega.value[2 * latter + 1][2 * former + 1] += -1.0/measurement_noise
            Omega.value[2 * latter + 1][2 * latter + 1] += 1.0/measurement_noise

            Xi.value[2 * former][0] += -Z[j][1]/measurement_noise
            Xi.value[2 * latter][0] += Z[j][1]/measurement_noise
            Xi.value[2 * former + 1][0] += -Z[j][2]/measurement_noise
            Xi.value[2 * latter + 1][0] += Z[j][2]/measurement_noise
