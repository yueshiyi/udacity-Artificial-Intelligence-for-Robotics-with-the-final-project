...
def doit(initial_pos, move1, move2):
    ...
    for i in range(2):
        former = i
        latter = i+1
        Omega.value[former][former] += 1
        Omega.value[former][latter] += -1
        Omega.value[latter][former] += -1
        Omega.value[latter][latter] += 1
        move = 0
        if i == 0: move = move1
        if i == 1: move = move2
        Xi.value[former][0] += -move
        Xi.value[latter][0] += move
    Omega.show('Omega: ')
    Xi.show('Xi: ')
    mu = Omega.inverse() * Xi
    mu.show('mu: ')
